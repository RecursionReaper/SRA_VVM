...
class Camera_Feed_Input(Node):    
    
    def __init__(self):
        super().__init__("Camera_Feed_Subscriber")
        Camera_Feed_Sub = self.create_subscription(CameraInfo, '/camera_info',self.Video_Input,qos_profile=qos.qos_profile_parameter_events)

    def Video_Input():
        Vid_Input = CameraInfo()
        # Write further code here!

def main(args=None):
    rclpy.init(args=args)
    node = Camera_Feed_Input()
    rclpy.spin(node)
    rclpy.shutdown()
...
class Object_Distance_Output(Node):    
    
    def __init__(self):
        super().__init__("Object_Distance_Publisher")
        Object_Pos_Pub = self.create_publisher(Float64MultiArray,'/obj_position',qos_profile=qos.qos_profile_parameter_events)
    # Creating Object
        Obj_Var = Float64MultiArray()
        Obj_Var.Data = [0.0,0.0,0.0,0.0]
    # Assigning Values
        Obj_Var.Data[0] = Obj_Variables["X_coord"]
        Obj_Var.Data[1] = Obj_Variables["Y_coord"]
        Obj_Var.Data[2] = Obj_Variables["Z_coord"]
        Obj_Var.Data[3] = Obj_Variables["distance"]
    # Publishing the distance of the object and its Coordinates
        Object_Pos_Pub.publish(Obj_Var)
