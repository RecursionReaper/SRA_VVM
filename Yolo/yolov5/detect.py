        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    label = names[c] if not hide_labels else ""
                    confidence_str = f"{conf:.2f}" if not hide_conf else ""
                    coords = f"Coords: ({xyxy[0].item():.2f}, {xyxy[1].item():.2f}), ({xyxy[2].item():.2f}, {xyxy[3].item():.2f})"
                    ...
                    im0 = cv2.putText(im0, coords, (int(xyxy[0].item()), int(xyxy[1].item() - 10)),
            ...
            if save_img:
                if dataset.mode == "image":
                    ...
                    if vid_path[i] != save_path:
                        ...
                        vid_writer[i] = cv2.VideoWriter(save_path, fourcc, 30, im0.shape[1::-1])  # FPS
                    vid_writer[i].write(im0)

    if save_txt or save_csv or save_crop:
        s += f"Results saved to {save_dir}{os.sep}"
    if update:
        strip_optimizer(weights)  # update model (to be implemented)

    LOGGER.info(f"Done. ({dt[0].dt:.3f}s, {dt[1].dt:.3f}s, {dt[2].dt:.3f}s)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project", type=str, default=ROOT / "runs/detect", help="save results to project/name")
